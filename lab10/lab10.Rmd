...
```{r}
p <- df %>% filter(League == 0) %>%
ggplot(aes(x =reorder(Team, Salary, decreasing = T) , y = Salary)) +
  geom_bar(stat = "identity", fill = "dodgerblue", alpha = 0.5) +
  labs(title = "Team Salary Comparison for the National League",
       x = "Team",
       y = "Total 2010 Team Salary ($Mil)") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  coord_cartesian(ylim = c(0, max(df$Salary)))
...
p <- ggplot(df, aes(x = Wins, y = Batting)) +
  ...
  labs(title = "Relationship between Wins and Batting Average",
       x = "Number of Wins",
       y = "Batting Average")
ggplotly(p)
```
...
p <- ggplot(df, aes(x = Wins, y = HR)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "red") +  
  labs(title = "Relationship between Wins and HR",
       x = "Number of Wins",
       y = "Batting Average")
ggplotly(p)
```
...
p <- ggplot(df, aes(x = Wins, y = Errors)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "red") +  
  labs(title = "Relationship between Wins and Errors",
       x = "Number of Wins",
       y = "Batting Average")
ggplotly(p)
```